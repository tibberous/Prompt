...
$AutoMode = "build-exes"
$AutoLaunchArgs = @("--build", "--offscreen", "--force-rebuild")
$AutoDescription = "build Prompt EXE artifacts with PyInstaller onefile, PyInstaller onedir, cx_Freeze, PyApp, and Nuitka"
...
$StatePath = Join-Path $Root (".auto-" + $AutoMode + "-state")
$CombinedLogPath = Join-Path $Root (".auto-" + $AutoMode + ".log")
...
function Stop-StalePromptBuildProcesses {
    if ($AutoMode -notmatch '(?i)build') { return }
    if ($env:PROMPT_AUTO_KILL_OLD_BUILDS -match '^(0|false|no|off)$') {
        Write-Log "Old build process cleanup skipped by PROMPT_AUTO_KILL_OLD_BUILDS=0"
        return
    }
    if (-not ($IsWindows -or $env:OS -match 'Windows')) { return }
    try {
        $rootText = [regex]::Escape($Root)
        $currentPid = $PID
        $processes = Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object {
            $_.ProcessId -ne $currentPid -and
            -not [string]::IsNullOrWhiteSpace([string]$_.CommandLine) -and
            ([string]$_.CommandLine -match $rootText) -and
            (
                ([string]$_.CommandLine -match '(?i)start\.py' -and [string]$_.CommandLine -match '(?i)--build') -or
                ([string]$_.CommandLine -match '(?i)tools[\\/]run_prompt_release\.py') -or
                ([string]$_.CommandLine -match '(?i)-m\s+PyInstaller' -and [string]$_.CommandLine -match '(?i)frozen_prompt_entry\.py') -or
                ([string]$_.CommandLine -match '(?i)nuitka' -and [string]$_.CommandLine -match '(?i)frozen_prompt_entry\.py')
            )
        }
        foreach ($proc in $processes) {
            Write-Log ("Stopping stale overlapping Prompt build PID {0}: {1}" -f $proc.ProcessId, ([string]$proc.CommandLine))
            try { & taskkill /PID $proc.ProcessId /T /F | Out-Null } catch { Write-Log ("WARN stale build taskkill failed PID {0}: {1}" -f $proc.ProcessId, $_.Exception.Message) }
        }
    } catch {
        Write-Log ("WARN stale build process scan failed: " + $_.Exception.Message)
    }
}
...
function Start-ChildProcess {
    ...
    if ($AutoMode -match '(?i)build') {
        Stop-StalePromptBuildProcesses
        if (-not $env:PROMPT_BUILD_SESSION_ID) { $env:PROMPT_BUILD_SESSION_ID = ([guid]::NewGuid().ToString("N")) }
    }
    ...
    if ($AutoMode -match '(?i)build') {
        ...
        if (-not $env:PROMPT_AUTO_INSTALL_BUILD_TOOLS) { $env:PROMPT_AUTO_INSTALL_BUILD_TOOLS = '1' }
        if (-not $env:PROMPT_AUTO_INSTALL_RUST) { $env:PROMPT_AUTO_INSTALL_RUST = '1' }
...
function Read-LogDelta {
    ...
    if ($combined.EndsWith("`n")) {
        $Partial.Value = ""
        $lines = $segments | Where-Object { $_ -ne "" }
    } else {
        if ($segments.Length -gt 1) {
            $Partial.Value = $segments[-1]
            $lines = $segments[0..($segments.Length - 2)] | Where-Object { $_ -ne "" }
        } else {
...
Write-Log "Mode: $AutoMode - $AutoDescription"
Write-Log ("Launch args: " + (Format-ArgumentString -Arguments $AutoLaunchArgs))
